...
import { GitCommit, GitDataService, DefaultGitDataService } from "./git-data-service";
...
export class SketchDiffRangePicker extends LitElement {
   ...
   */
  formatCommitOption(commit: GitCommit): string {
  ...
  async loadCommits() {
    ...
    try {
      ...
      if (this.commits.length > 0) {
        ...
        this.toCommit = this.commits[0].hash; // HEAD is usually first
