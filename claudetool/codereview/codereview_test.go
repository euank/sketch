...
// TestCodereviewTimeout tests that the timeout functionality works correctly.
func TestCodereviewTimeout(t *testing.T) {
	...
	go1Content := []byte(`package main

func main() {
	println("hello world")
}
`)
	if err := os.WriteFile(filepath.Join(tmpDir, "main.go"), go1Content, 0644); err != nil {
...
`)
	if err := os.WriteFile(filepath.Join(tmpDir, "main.go"), go2Content, 0644); err != nil {
		t.Fatalf("failed to write main.go: %v", err)
	}
	...
	reviewer, err := NewCodeReviewer(context.Background(), tmpDir, initialCommit)
	if err != nil {
		t.Fatalf("failed to create code reviewer: %v", err)
	}

	// Test with very short timeout (1 nanosecond) - this should timeout
	tool := reviewer.Tool()
	input := []byte(`{"timeout":"1ns"}`)
	...
	// Check that the error is indeed a timeout error
	if !strings.Contains(err.Error(), "context deadline exceeded") {
		t.Fatalf("expected timeout error, got: %v", err)
	}

	// Test with reasonable timeout (10 seconds) - this should work
	input = []byte(`{"timeout":"10s"}`)
	_, err = tool.Run(context.Background(), input)
	if err != nil {
		// Allow for some expected errors (like missing tools), but not timeout
		if strings.Contains(err.Error(), "context deadline exceeded") {
			t.Fatalf("unexpected timeout error with 10s timeout: %v", err)
		}
		// Other errors are OK (like missing gopls, etc.)
		t.Logf("Non-timeout error (expected): %v", err)
	}
	...
	if err != nil {
		// Allow for some expected errors (like missing tools), but not timeout
		if strings.Contains(err.Error(), "context deadline exceeded") {
			t.Fatalf("unexpected timeout error with default timeout: %v", err)
		}
		// Other errors are OK (like missing gopls, etc.)
		t.Logf("Non-timeout error (expected): %v", err)
	}
